...
func createMockImagenSuccessResponse(base64EncodedImage: String, mimeType: String = "image/png") -> Data {
    let responseJSON = """
    {
        "predictions": [
            {
...
func setupMockResponses(urlSession: URLSession, apiKey: String = "test_key") {
    ...
    guard let configuration = (urlSession as? URLSession)?.configuration,
          ...
          protocolClasses.contains(where: { $0 == MockURLProtocol.self }) else {
        fatalError("The provided URLSession is not configured with MockURLProtocol")
    }
    
    // Base API URL that contains our model name
    let baseURL = "https://generativelanguage.googleapis.com/v1beta/models/imagen-3.0-generate-002:predict?key=\(apiKey)"
    ...
    MockURLProtocol.registerMock(for: baseURL) { request in
        ...
        let defaultBase64Image = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII="
