...
struct CollectionFormView: View {
    ...
    // State for validation - interests are now optional
    private var isInterestsValid: Bool {
        true // Always valid since interests are now optional
    }
    ...
    private func generateCollection() async {
        ...
        // No validation needed since interests are now optional
        ...
        let parameters = CollectionParameters(
            ...
            interests: interests.isEmpty ? "General interests" : interests,
        ...
        FormFieldContainer {
            HStack {
                Image(systemName: "star.fill")
                    .font(.system(size: 14))
                    .foregroundColor(Color(hex: "#7B61FF"))
                Text("Child's Interests (Optional)")
                    .font(.system(size: 16, weight: .semibold))
            }
