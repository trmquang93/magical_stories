...
struct PaywallView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button(R.string.localizable.navigationClose()) {
                        dismiss()
                    }
        ...
        .task {
            ...
            // Track paywall view
            analyticsService.trackScreenView(ClarityConfiguration.ScreenNames.paywall)
    ...
    private var subscriptionOptionsSection: some View {
        VStack(spacing: 16) {
            Text(R.string.localizable.paywallTitle())
                .font(.headingLarge.weight(.semibold))
                .foregroundColor(.magicalTextPrimary)
            
            if purchaseService.isLoading {
                ProgressView(R.string.localizable.paywallLoading())
                    .foregroundColor(.magicalTextSecondary)
            } else if purchaseService.products.isEmpty {
                VStack(spacing: 8) {
                    Text(R.string.localizable.paywallErrorLoading())
                        .foregroundColor(.magicalError)
                    
                    Button(R.string.localizable.actionRetry()) {
        ...
        VStack(spacing: 12) {
            HStack(spacing: 24) {
                TrustIndicator(
                    ...
                    text: R.string.localizable.paywallTrustSecure()
                )
                
                TrustIndicator(
                    icon: "arrow.clockwise",
                    text: R.string.localizable.paywallTrustCancel()
                )
                
                TrustIndicator(
                    icon: "checkmark.seal.fill",
                    text: R.string.localizable.paywallTrustNoFees()
                )
            }
        }
        .padding(.vertical, 8)
    ...
    private var actionButtonsSection: some View {
        VStack(spacing: 16) {
            ...
            Button(R.string.localizable.paywallRestore()) {
                Task {
                    do {
    ...
    private var legalLinksSection: some View {
        HStack(spacing: 16) {
            Button(R.string.localizable.paywallTerms()) {
                showingTerms = true
            }
            ...
            Button(R.string.localizable.paywallPrivacy()) {
                showingPrivacy = true
            }
        }
        .foregroundColor(.magicalTextSecondary)
        ...
        .sheet(isPresented: $showingTerms) {
            SafariView(url: URL(string: AppConfig.termsOfUseURL)!)
        }
        .sheet(isPresented: $showingPrivacy) {
            SafariView(url: URL(string: AppConfig.privacyPolicyURL)!)
        }
    }
    ...
    private var primaryButtonText: String {
        guard let product = selectedProduct else {
            return R.string.localizable.paywallSelectPlan()
        }
        
        if let trialText = product.introductoryOfferText {
            return R.string.localizable.paywallStartTrial(trialText)
        } else {
            return R.string.localizable.paywallSubscribeFor(product.displayPrice)
        }
    }
