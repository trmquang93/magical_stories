...
class EntitlementManager: ObservableObject {
    ...
    private var usageAnalyticsService: (any UsageAnalyticsServiceProtocol)?
    ...
    func setUsageAnalyticsService(_ usageAnalyticsService: any UsageAnalyticsServiceProtocol) {
    ...
    open func canGenerateStory() async -> Bool {
        #if DEBUG
        // In DEBUG mode, always allow story generation (bypass usage limits)
        return true
        #else
        ...
        guard let usageTracker = usageTracker else {
            // If no usage tracker is set, allow generation (default behavior for new users)
            logger.debug("No usage tracker set - allowing story generation for free user")
            return true
        }
        
        return await usageTracker.canGenerateStory()
        #endif
    }
    ...
    open func getRemainingStories() async -> Int {
        ...
        guard let usageTracker = usageTracker else {
            // If no usage tracker is set, return default free tier limit for new users
            return FreeTierLimits.storiesPerMonth
        }
        
        return await usageTracker.getRemainingStories()
    }
    ...
    private func processSubscriptionTransaction(_ transaction: Transaction, product: SubscriptionProduct, calculatedExpiration: Date) async {
        ...
        // Record subscription activation for rating system (non-blocking)
