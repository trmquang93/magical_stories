...
enum AIError: Error, LocalizedError, Identifiable {
    ...
    case apiError(String, (any Error)?)
    ...
    var errorDescription: String? {
        ...
        switch self {
        case .networkError(_):
            description = "Network connection problem. Please check your internet connection and try again."
        case .apiError(let message, _):
            description = "There was a problem with the AI service: \(message)"
        case .configurationError(let message):
            description = "App configuration error: \(message)"
        case .textGenerationFailed(let message):
            description = "Failed to generate story: \(message)"
        case .invalidTextGenerationParameters(let message):
            description = "Invalid story parameters: \(message)"
        case .contentFilterTriggered(_):
            description = "The story couldn't be generated because the content was flagged by safety filters."
        case .imageGenerationFailed(let message):
            description = "Failed to generate illustration: \(message)"
        case .noImageDataReturned:
            description = "The illustration service didn't return any image data."
        case .imageProcessingError(_):
            description = "There was a problem processing the generated illustration."
        case .resourceUnavailable(let message):
            description = "Required resource is unavailable: \(message)"
        }
        
        // Truncate error descriptions that are too long (max 1000 characters)
        if description.count > 1000 {
            let truncated = String(description.prefix(997)) + "..."
            return truncated
        }
    ...
    static func from(_ error: any Error) -> AIError {
...
class AIErrorManager {
    ...
    static func logError(_ error: any Error, source: String, additionalInfo: String? = nil) {
    ...
    static func createAlert(for error: any Error, retryAction: (() -> Void)? = nil) -> Alert {
        let aiError = AIError.from(error)
